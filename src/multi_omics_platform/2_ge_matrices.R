...
createMatrixReport <- function(output.path, phenos, col.env.id, 
  ...
  if (is.null(markers)) {
    ...
    save(G, file = paste0(output_path, '/output/G.rda'))
    save(EVD, file = paste0(output_path, '/output/EVD.rda'))
  ...
  return(paste0("The results were saved in the following folder: ", output.path, "/output/"))
