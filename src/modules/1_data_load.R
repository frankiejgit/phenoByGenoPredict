...
createNaNFiles <- function(phenos, markers, nan.freq) {
  ...
  write.table(NaN.freq, file = paste0(output.path,'NaNs_freq.csv'), sep = ',', row.names = FALSE, col.names = FALSE)
  cat("Saved the NaN.freq file in the tmp directory\n")
  
  write.table(percentage.NaNs, file = paste0(output.path, 'percentage_NaNs.csv'), sep = ',', row.names = FALSE, col.names = FALSE)
  ...
  write.csv(no.NaNs.markers, file = paste0(output.path, 'no_nans_markers.csv'), row.names = FALSE)
